    ...
    <div class="small-box">
      <el-row class="rowLayout" :gutter="10">
        <el-col :span="8" v-for="(item, index) in house" :key="index">
          <el-card v-if="(index < numOrders * curPage) && (index >= numOrders * (curPage - 1))"
            ...
            <div class="card-content">
              <h2>房间大小:{{ item.size }}</h2>
            </div>
            <div class="card-content" style="font-size:14px;">房间价格：<span style="color:red">{{ item.price }}</span>/(平方米)
            </div>
          </el-card>
        </el-col>
      </el-row>
